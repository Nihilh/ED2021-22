...
public class AVLTree <T extends Comparable<T>>{
	...
	/**
	 * Método que devuelve los nodos que hay entre dos claves
	 * 
	 * @param clave1 clave de inicio
	 * @param clave2 clave del final
	 * 
	 * @return Cadena con el recorrido de los nodos
	 */
	public String caminoNodos(T clave1, T clave2) {
		if(clave1==null || clave2==null || raiz ==null 
				|| clave1.compareTo(clave2)==0) return "";
		AVLNode<T> nodo = this.searchNode(clave1);
		return nodosCaminoR(nodo,clave2);
		
	}
	
	/**
	 * Método que devuelve el camino de nodos 
	 * 
	 * @param nodo nodo de inicio
	 * @param clave2 nodo de destino
	 * 
	 * @return Cadena con los nodos
	 */
	private String nodosCaminoR(AVLNode<T> nodo, T clave2) {	
		String nodos="";
		nodos+=nodo.getInfo()+"\t";
		if(nodo.getInfo().compareTo(clave2)==0) return nodos;
		else if (nodo.getInfo().compareTo(clave2)<0)
			nodos+=nodosCaminoR(nodo.getRight(), clave2);
		else if (nodo.getInfo().compareTo(clave2)>0)
			nodos+=nodosCaminoR(nodo.getLeft(), clave2);
		return nodos;
	}
	
	/**
	 * Método que devuelve la información del nodo mayor
	 * @return información del nodo mayor
	 */
	public T maximoNodo() {
		if(raiz==null) return null;
		return maximoNodoR(raiz);
	}

	/**
	 * Método recursivo que devuelve la información del nodo mayor
	 * 
	 * @param nodo nodo del que empezar a buscar
	 * @return nodo mayor
	 */
	private T maximoNodoR(AVLNode<T> nodo) {
		if(nodo== null) return null;
		if(nodo.getRight()!=null)
			return maximoNodoR(nodo.getRight());
		return nodo.getInfo();
	}
	
	/**
	 * Método que devuelve la información del nodo menor
	 * @return información del nodo menor
	 */
	public T minimoNodo() {
		if(raiz==null) return null;
		return minimoNodoR(raiz);
	}

	/**
	 * Método recursivo que devuelve la información del nodo menor
	 * 
	 * @param nodo nodo del que empezar a buscar
	 * @return nodo menor
	 */
	private T minimoNodoR(AVLNode<T> nodo) {
		if(nodo== null) return null;
		if(nodo.getLeft()!=null)
			return minimoNodoR(nodo.getLeft());
		return nodo.getInfo();
	}
