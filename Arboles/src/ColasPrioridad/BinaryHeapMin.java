...
public class  BinaryHeapMin<T extends Comparable<T>>implements PriorityQueue<T>{
	...
	/**
	 * Método que intercambia el padre por el hijo menor.
	 * 
	 * @param pos posición del padre.
	 * @return posición del hijo menor.
	 */
	private int cambiarHijoMenor(int pos) {
	...
	/*
	 * Método que devuelve el máximo del monticulo
	 * 
	 * @return Elemento de valor máximo
	 */
	public T maximo() {
		T max=monticulo[numElementos/2];
		for(int i=numElementos/2;i<numElementos;i++) {
			if(monticulo[i].compareTo(max)>0) max= monticulo[i];
		}
		return max;
	}
	
	/*
	 * Método que devuelve la altura del monticulo
	 */
	public int altura() {
		return (int) ((Math.log(numElementos)/Math.log(2))+1);
	}
